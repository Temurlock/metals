...
class CompletionLspSuite extends BaseCompletionLspSuite("completion") {
  ...
  test("scope ranking on file change removed usage") {
    cleanWorkspace()
    for {
      _ <- initialize(
        """/metals.json
      ...
      _ = println("saving B")
      _ <- server.didSave("a/src/main/scala/a/B.scala")(identity)
      _ = assertNoDiagnostics()
      _ = println("change A")
      _ <- server.didChange("a/src/main/scala/a/A.scala")(_ =>
        """|package a
  ...
  // TODO:
  //  1) добавить тест с удалением файла
}
